...
function stream_xhttp_extra(server) {
    let extra = {};
    // Layer 1: raw JSON from subscription (preserves unknown fields like
    // xPaddingObfs / xmux / headers). Silently ignored if not parseable.
    let raw = server["xhttp_extra_json"] || "";
    if (raw !== "") {
        try {
            let parsed = json(raw);
    ...
    // Layer 2: whitelisted UCI fields override raw, so edits made in the UI
    // take precedence over what came in from the subscription URL.
    let v = server["xhttp_extra_sc_max_concurrent_posts"] || "";
    if (v !== "") extra["scMaxConcurrentPosts"] = int(v);
    v = server["xhttp_extra_sc_max_each_post_bytes"] || "";
    if (v !== "") extra["scMaxEachPostBytes"] = int(v);
    v = server["xhttp_extra_sc_min_posts_interval_ms"] || "";
    if (v !== "") extra["scMinPostsIntervalMs"] = int(v);
    if (server["xhttp_extra_no_sse_header"] === "1") extra["noSSEHeader"] = true;
    v = server["xhttp_extra_x_padding_bytes"] || "";
    if (v !== "") extra["xPaddingBytes"] = v;
    return length(extra) > 0 ? extra : null;
}

function stream_xhttp_download(server) {
    let addr = server["xhttp_download_address"] || "";
    if (addr === "") return null;
    let inner = {};
    let p = server["xhttp_download_path"] || "";
    let h = server["xhttp_download_host"] || "";
    if (p !== "") inner["path"] = p;
    if (h !== "") inner["host"] = h;
    let port = server["xhttp_download_port"] || "";
    return {
        address: addr,
...
function stream_xhttp(server) {
    if (server["transport"] === "xhttp") {
        ...
        let mode = server["xhttp_mode"] || "";
        if (mode !== "" && mode !== "auto") result["mode"] = mode;
        let extra = stream_xhttp_extra(server);
        if (extra !== null) result["extra"] = extra;
        let download = stream_xhttp_download(server);
        if (download !== null) result["downloadSettings"] = download;
        return result;
    }
    return null;
}
...
function stream_hysteria(server) {
    if (server["transport"] === "hysteria") {
        ...
        let udp_idle_timeout = server["hysteria_udp_idle_timeout"] || "";
        if (udp_idle_timeout !== "") {
            result["udpIdleTimeout"] = int(udp_idle_timeout);
        }
...
// Salamander obfuscation + QUIC tuning, mirroring the "finalmask" stream object
// used by the hysteria2-capable Xray fork (see re-ui). Emitted only when the
// user actually set something, so default hysteria configs stay byte-identical.
function stream_finalmask(server) {
    if (server["transport"] !== "hysteria") {
        return null;
    }

    let udp = [];
    if (server["hysteria_obfs_type"] === "salamander") {
        let password = server["hysteria_obfs_password"] || "";
        if (password !== "") {
            push(udp, {
                type: "salamander",
    ...
    let quic = {};
    let congestion = server["hysteria_congestion"] || "";
    if (congestion !== "") {
        quic["congestion"] = congestion;
        if (congestion === "bbr" && (server["hysteria_bbr_profile"] || "") !== "") {
            quic["bbrProfile"] = server["hysteria_bbr_profile"];
        }
        if (congestion === "brutal" || congestion === "force-brutal") {
            if ((server["hysteria_brutal_up"] || "") !== "") quic["brutalUp"] = server["hysteria_brutal_up"];
            if ((server["hysteria_brutal_down"] || "") !== "") quic["brutalDown"] = server["hysteria_brutal_down"];
        }
    }

    // Map UCI option -> quicParams key. All optional integers.
    const int_params = {
    ...
    for (let k in int_params) {
        let v = server[k] || "";
        if (v !== "") quic[int_params[k]] = int(v);
    }
    if (server["hysteria_quic_disable_mtu_discovery"] === "1") quic["disablePathMTUDiscovery"] = true;
    if (server["hysteria_quic_debug"] === "1") quic["debug"] = true;

    let result = {};
    if (length(udp) > 0) result["udp"] = udp;
    if (length(keys(quic)) > 0) result["quicParams"] = quic;
...
export function stream_settings(server, protocol, tag) {
    ...
    return {
        stream_settings: {
            ...
            finalmask: stream_finalmask(server)
        },
