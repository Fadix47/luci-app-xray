...
import { dns_conf, dns_rules, dns_server_inbounds, dns_server_outbounds, fast_domain_rules, secure_domain_rules } from "./feature/dns.mjs";
...
function outbounds(proxy, config, manual_tproxy, bridge, extra_inbound, fakedns) {
    ...
    for (let i in keys(outbound_balancers_all)) {
        ...
        let parts = split(i, ":");
        let section_name = parts[length(parts) - 1];
        push(result, ...server_outbound(config[section_name], i, config));
    }
    return result;
}
...
function rules(proxy, bridge, manual_tproxy, extra_inbound, fakedns) {
    ...
    let result = [
        ...
        // `geoip:private` direct rule — only when geo is loaded (else xray refuses config).
        ...((geoip_existence && proxy["geo_disabled"] != "1") ? [{
            type: "field",
    ...
    if (proxy["tproxy_sniffing"] == "1") {
        ...
        // Blocked-domains routing rule was removed along with the rest of
        // the blocked-domain UI surface.
        splice(result, 0, 0, {
            type: "field",
